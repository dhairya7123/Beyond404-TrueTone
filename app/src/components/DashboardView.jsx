...
export default function DashboardView({ go, startCall, user, contactsCount, activities = [] }) {
  ...
  function ActivityRow({ r }) {
    ...
    return (
      <tr className="border-t border-white/70">
        ...
        <td className="py-3">
          {r.status === "missed" ? (
            <span className="neu-icon-slate text-slate-500 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
              Missed
            </span>
          ) : r.status === "declined" ? (
            <span className="neu-icon-amber text-amber-600 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
              Declined
            </span>
          ) : r.flagged ? (
            <span className="neu-icon-red text-red-600 px-3 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1 whitespace-nowrap">
              <AlertTriangle size={12} /> AI Clone Flagged
            </span>
          ) : (
            <span className="neu-icon-green text-emerald-600 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
              Verified Genuine
            </span>
          )}
        </td>
  ...
  function ActivityMobileCard({ r }) {
    const isOutgoing = r.caller_id === user?.id;
    const otherName = isOutgoing ? r.callee_name : r.caller_name;
    const otherPhone = isOutgoing ? r.callee_phone : r.caller_phone;

    return (
      <div className="neu-card-sm p-3.5 flex flex-col gap-2">
        ...
        <div>
          <div className="font-medium text-slate-700 text-sm">{otherName}</div>
          <div className="text-xs text-slate-400">{otherPhone}</div>
        </div>
        <div>
          {r.status === "missed" ? (
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 mb-8">
        <div className="min-w-0">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 break-words">
            Good morning, {user?.name || "Analyst"}.
          </h1>
        ...
        <div className="flex flex-col xs:flex-row sm:flex-row gap-3 w-full sm:w-auto">
          <div className="neu-card p-5 w-full sm:w-48 flex items-center gap-3">
          ...
          <div className="neu-card p-5 w-full sm:w-64 flex items-center gap-3">
          ...
          <div className="flex items-center gap-4 w-full text-left">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-baseline gap-2 mt-0.5">
                ...
                <div className="flex items-center gap-0.5 text-xs font-bold text-emerald-500 whitespace-nowrap">
          ...
          <div className="flex items-center gap-4 w-full text-left">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-baseline gap-2 mt-0.5">
                ...
                <div className="flex items-center gap-0.5 text-xs font-bold text-red-500 whitespace-nowrap">
          ...
          <div className="flex items-center gap-4 w-full text-left">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-baseline gap-2 mt-0.5">
                ...
                <div className="flex items-center gap-0.5 text-xs font-bold text-blue-500 whitespace-nowrap">
                  Total
                </div>
              </div>
        ...
        <div className="neu-card p-6 xl:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-2 font-semibold text-slate-800">
              <span className="neu-icon-slate w-9 h-9 rounded-full flex items-center justify-center shrink-0">
        ...
        <div className="neu-card p-6 xl:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-2 font-semibold text-slate-800 min-w-0">
              ...
              <span className="truncate">Recent Call Activity (For {user?.name})</span>
            </div>
            <button onClick={() => go("reports")} className="text-sm text-blue-600 font-medium flex items-center gap-1 hover:underline shrink-0">
          ...
          <div className="sm:hidden flex flex-col gap-3">
            {userActivities.map((r) => (
              <ActivityMobileCard key={r.id} r={r} />
            ))}
            {userActivities.length === 0 && (
