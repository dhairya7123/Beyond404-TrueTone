...
export function UploadAudio({ onAnalyzed, currentUser }) {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      const targetUrl = apiUrl("/api/analyze-audio");
      ...
      const score = Math.round(data.overall_risk_score ?? data.score ?? 50);
      ...
      onAnalyzed({
        ...
        model_engine: data.model_engine || "local_model (Wav2Vec2)",
      });
    } catch (err) {
      console.error("Audio analysis error:", err);
      setErrorMsg("Analysis error: " + err.message + ". Please verify Backend URI in settings.");
    } finally {
      setAnalyzing(false);
    }
  }
          ...
          {data.plot_image && (
            <div className="neu-card p-6 mt-6">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Standardized Waveform &amp; Log-Mel Spectrogram</h3>
                  <p className="text-xs text-slate-500">Processed by Backend Wav2Vec2 Inference Engine</p>
                </div>
                <a
                  href={data.plot_image}
                  download={data.name.replace(/\.[^.]+$/, "") + "-mel-spectrogram.png"}
                  className="neu-card-sm neu-press flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full text-blue-600"
                >
                  <Download size={13} /> Spectrogram PNG
                </a>
              </div>
              <div className="neu-inset rounded-2xl overflow-hidden bg-slate-950 p-2">
                <img
                  src={data.plot_image}
                  alt="Standardized Waveform and Spectrogram"
...
export function Reports({ search, setSearch, onOpen, activities = [], currentUser }) {
  ...
  return (
    <div className="neu-card p-6">
      ...
      {/* Desktop / tablet table — unchanged, sm and up */}
        ...
        {filtered.map((r, i) => {
          ...
          return (
            <div
              ...
              className="neu-card-sm p-4 flex flex-col gap-2.5 cursor-pointer"
            >
              ...
              <div>
                <div className="text-slate-700 font-medium text-sm">{otherName}</div>
                <div className="text-xs text-slate-400">{otherPhone}</div>
              </div>
